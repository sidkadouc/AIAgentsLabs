...
class WeatherPlugin:
    ...
    def get_weather(self, city: str) -> str:
        ...
        with tracer.start_as_current_span("weather_plugin.get_weather_group") as span:
            span.set_attribute("city", city)
            custom_metrics['function_call_counter'](1, {"function": "get_weather", "plugin": "weather_group"})
            ...
            if "paris" in city.lower():
                ...
                result = f"The weather in {city} is 5°C and cloudy."
            else:
                result = f"The weather in {city} is 30°C and cloudy."
            
            span.set_attribute("result_length", len(result))
            logging.info(f"Group weather request processed for city: {city}")
            return result
...
def create_hotel_concierge_group_chat(kernel):
    ...
    with tracer.start_as_current_span("group_chat.create") as span:
        ...
        span.set_attribute("reviewer_name", REVIEWER_NAME)
        span.set_attribute("frontdesk_name", FRONTDESK_NAME)
        span.set_attribute("weather_name", WEATHER_NAME)
        
        logging.info("Hotel concierge group chat created with 3 agents")
